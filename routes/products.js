        ...
        if(err){
            ...
            jwt.verify(token, data.password, function(err, decoded) {
                if(err){
                    ...
                    ProductModel.find().exec().then((data, err)=>{
                        if(err){
                            console.log(err);                            
        ...
        if(err){
            ...
            jwt.verify(token, data.password, function(err, decoded) {
                if(err){
                    ...
                    ProductModel.findOneAndUpdate({"_id":req.body._id},req.body,{upsert:true}).exec().then((data, err)=>{
                        if(err){
                            res.status(400).send({"message": "failed", "Error" : err})
                        }
                        else{
        ...
        if(err){
            ...
            jwt.verify(token, data.password, function(err, decoded) {
                if(err){
                    ...
                    // data = new ProductModel(req.body)
                    ProductModel.findOneAndDelete({"_id":req.query._id}).exec().then((data, err)=>{
                        if(err){
                            console.log(err);                            
                            res.status(400).send({"message": "failed", "Error" : err})
                        }
